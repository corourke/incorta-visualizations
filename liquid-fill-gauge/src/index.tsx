...
const LiquidFillGauge = (props: ComponentProps) => {
  ...
  const insightData = props.response.data;
  const [aggregationData] = insightData;

  const formattedMeasures = props.context.component.bindings.measure.map((col, index) => {
  ...
  return (
    <div className="LiquidFillGauge__wrapper">
      {formattedMeasures.map(measure => {
        let chartConfig = {
          ...measure,
          color: measure.color ?? props.context.app.color_palette[0]
        };
