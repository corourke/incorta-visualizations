...
const IframeLiquidFillGauge = ({ context, data: resData }: Props) => {
  const insightData = resData.data;
  const [aggregationData] = insightData;

  const formattedMeasures = context.component.bindings.measure.map((col, index) => {
  ...
  return (
    <div className="LiquidFillGauge__wrapper">
      {formattedMeasures.map(measure => {
        let chartConfig = {
          ...measure,
          color: measure.color ?? context.app.color_palette[0]
        };
...
export default IframeLiquidFillGauge;
